...
function Doc(connection, collection, id) {
  ...
  this.connection.dataInterface.delete(collection, id);
...
// Pass data through the interface
Doc.prototype.getData = function() {
  var storeSnapshot = this.connection.dataInterface.select(this.collection, this.id);
  return storeSnapshot && storeSnapshot.data;
}
Doc.prototype.setData = function(data) {
  this.connection.dataInterface.write({
    id: this.id,
  ...
  if (op.op) {
    ...
    if (!source && this.type === types.defaultType && op.op.length > 1) {
      ...
      for (var i = 0; i < op.op.length; i++) {
        ...
        docData = this.type.apply(docData, componentOp.op);
        this.emit('op', componentOp.op, source);
      }
      // Pop whatever was submitted since we started applying this op
      this._popApplyStack(stackLength);
      this.setData(docData);
      return;
    }
